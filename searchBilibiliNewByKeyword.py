...
def getBiliBiliVideo(url,p,bv):
    ...
    if flag == 0:
        ...
        combineVideoAudio('./result/' + name + '_Video.mp4','./result/' + name + '_Audio.mp3','./result/' + name + '_output.mp4')
...
def fileDownload(homeurl,url, name, session=requests.session()):
    ...
    while True:
        ...
        print('bytes=' + str(begin) + '-' + str(end))
        # 获取视频分片

        res = session.get(url=url, headers=headers,verify=False)

        if res.status_code != 416 and res.status_code != 200:

            # 响应码不为为416时有数据
            print("processing", res.status_code)
            begin = end + 1
